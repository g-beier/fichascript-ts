...
<template>
  <Popover v-slot="{ open }" class="relative">
    <PopoverButton
      ...
      class="flex w-full flex-col items-center rounded-md border border-gray-400 bg-white px-3 py-2 font-medium tracking-tight text-gray-500 hover:text-opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75"
    >
      <span>{{ props.modelValue.label }}</span>
      <span>{{
        (props.modelValue.mod() > 0
          ? "+"
          : props.modelValue.mod() < 0
          ? "-"
          : "") + props.modelValue.mod()
      }}</span>
    </PopoverButton>
